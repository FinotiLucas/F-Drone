...
from src.views import DFormUTM
import json
...
class TabelaEntrada(QWidget):
    ...
    def __init__(self, parent, dados):
        ...
        self.pai.setCentralWidget(self.table)
    ...
    def ExibirTabela(self, dados):
        ...
        self.table.cellClicked.connect(self.CelulaClicada)
    ...
    def CelulaClicada(self, linha, coluna):
        if(self.adicionar_coordenada and coluna == 1):
            self.table.item(linha, coluna).setBackground(QColor(100,100,150))
            DFormUTM.DialogoFormUTM(self, self.table.item(linha, coluna).text())
